...
def test_scoring_produces_valid_normalized_ranking() -> None:
    ...
    assert ranked_names.index("Triticum aestivum (Cuce Bugday)") < ranked_names.index("Solanum lycopersicum (Dwarf Domates)")
...
def test_environment_influence_changes_recommendation_behavior() -> None:
    ...
    assert moon.top_crops[0].name == "Lactuca sativa (Marul)"
